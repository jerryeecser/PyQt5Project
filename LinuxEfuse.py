...
class service_connect(QThread):
    efuse_sts = pyqtSignal(str)
    ...
    def run(self):
        ...
        for _ in range(10):
            if client.get_connection_state() == NrpcConnectionState.kConnected:
                ...
                # self.receive_area.append(f"Success to connect service {service_name!r}.")
                break

            print("Wait for connecting service...")
            # self.receive_area.append(f"Wait for connecting service...")
            time.sleep(1)
        else:
            print(
...
class basicsurf(QWidget):
    ...
    def EfuseStsMsg(self, msg):
        msg = list(msg)
        for i in len(msg):
            if i > 9:
                break
            if "ON" in msg[i]:
                self.V2label[i].setStyleSheet("background-color: rgb(0, 255, 0)")  
            elif "OFF" in msg[i]:
                self.V2label[i].setStyleSheet("background-color: rgb(200, 200, 200)")

    #各种信号槽函数处理
    def SignalProcessing(self):
        self.efusethread = service_connect() #efuse显示处理
        self.efusethread.efuse_sts.connect(self.EfuseStsMsg)
        self.efusethread.start()
